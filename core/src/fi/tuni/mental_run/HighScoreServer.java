 ...
 */
public class HighScoreServer extends GameProject {
    /**
     * url is the address of the high score server.
     */
    private static String url;

    /**
     * Password for the high score host.
     */
    private static String password;

    /**
     * Username for the high score host.
     */
     ...
     * @param levelVariable tells which levels' scores to fetch
     */
    public static void fetchHighScores(final fi.tuni.mental_run.HighScoreListener source, int levelVariable) {
        ...
        try {
            ...
            URL connectionURL = new URL(url + "?level=" + levelVariable);
            ...
            if (urlConnection.getResponseCode() == 200) {
                ...
                ArrayList<fi.tuni.mental_run.HighScoreEntry> highScores = new ArrayList<>();

                for (int i = 1; i <= 10; i++) {
                    JsonValue entry = jsonObject.get(String.valueOf(i));
                    if (entry != null) {
                        fi.tuni.mental_run.HighScoreEntry score = new fi.tuni.mental_run.HighScoreEntry(
     ...
     *
     * @param levelVariable tells which levels' scores to send
     * @param highScore     The new HighScoreEntry data to be sent to the server
     * @param source        source class implementing HighScoreListener
     */
    public static void sendNewHighScore(int levelVariable, HighScoreEntry highScore, final HighScoreListener source) {
        HttpsURLConnection urlConnection = null;
        try {
            CertificateFactory cf = CertificateFactory.getInstance("X.509");
            // Properties prop = new Properties();
            ...
            writer.write(String.format("level=%d&name=%s&score=%d&user=%s&password=%s", levelVariable, highScore.getName(), highScore.getScore(), user, password));
            writer.flush();
            writer.close();
            os.close();
